...
# 3. Login (OAuth — opens browser, auto-registers API key)
cobo login -u                          # Authorize in browser when prompted
# 4. Generate API keys
cobo keys generate --key-type API      # Secret saved automatically
# 5. Register key
cobo keys register        # dev/sandbox only — auto-registers
# For prod: cobo open developer → add public key manually + grant permissions
# 6. Set auth method
cobo auth apikey
# 7. Test it works
...
## State Detection
**Before starting, run these checks and fix any issues automatically — do not ask the user to run commands manually.**
```bash
# Is CLI installed?
cobo version
# Is it configured?
...
```
| Result | State | Action (execute automatically) |
|--------|-------|--------------------------------|
| `command not found` | CLI not installed | Run `pip install cobo-cli` |
| Config empty | Not configured | Run Quick Start steps automatically |
| `Key or secret not found` | Keys missing | Run `cobo login -u` → `cobo keys generate --key-type API` → `cobo keys register` |
| 401 error | Keys not registered | dev/sandbox: run `cobo keys register` / prod: run `cobo open developer` and notify user to add key + grant permissions |
| error_code 2024 | Unauthorized | Run `cobo login -u` then re-register keys |
| JSON response | Ready | Proceed with task |
**Execution principle: always run the fix commands yourself. Only stop and ask the user when human interaction is unavoidable (e.g. browser OAuth login, manual Portal permission grant in prod).**
...
## Recovery Patterns
When operations fail, **execute the fix automatically** — do not show the user a list of commands to run manually.
...
4. **Apply fix** - Execute the recovery commands directly
5. **Verify** - Re-run the original operation and confirm success
